...
test_that("functions returns a list", {
  ...
  expect_type(tnl(7,5, 1), "list")
  expect_type(tnl.sim(5,7, 1), "list")
  expect_type(ptnl(2, 5,6, 1), "list")
...
test_that("functions returns the right output", {
  expect_equal(sum(tnl(7,10, 1)$pmf), 1)
  expect_equal(sum(tnl.sim(4,5, 1)$pmf), 1)
  expect_equal(tnl(7,9, 1)$cdf[7], 1)
  expect_equal(tnl.sim(4,4, 1)$cdf[4], 1)
  expect_equal(ptnl(5, 5,5, 2)$cdf, 1)
  expect_equal(ptnl.lehmann(6, 6,8, 2, 0.5), 1)
})
